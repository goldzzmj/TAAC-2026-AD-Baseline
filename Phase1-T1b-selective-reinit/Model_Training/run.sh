...
# Phase1-T1b: Selective sparse re-init
# Only change vs Phase0 baseline:
#   reinit_sparse_after_epoch=2 (delay re-init to epoch 2)
#   reinit_cardinality_threshold=100000 (only re-init high-cardinality embeddings)
#
...
"${PYTHON_BIN_CMD[@]}" -u "${SCRIPT_DIR}/train.py" \
    --run_name "${RUN_NAME:-phase1_t1b_selective_reinit}" \
