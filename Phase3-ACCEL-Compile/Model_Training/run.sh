...
# Phase3-ACCEL-Compile: torch.compile training acceleration
# Only change vs T2 baseline (cosine LR):
#   --use_compile  (torch.compile model wrapping)
#
# Goal: confirm torch.compile does NOT degrade AUC while speeding up training.
...
"${PYTHON_BIN_CMD[@]}" -u "${SCRIPT_DIR}/train.py" \
    --run_name "${RUN_NAME:-phase3_accel_compile}" \
