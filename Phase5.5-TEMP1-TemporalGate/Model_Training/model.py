...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        if use_temporal_gate and num_time_buckets > 0:
    ...
    def _embed_seq_domain(
        ...
        # Add time bucket embedding
        if self.num_time_buckets > 0:
            time_emb = self.time_embedding(time_bucket_ids)  # (B, L, D)
            if self.use_temporal_gate:
                # Multiplicative gate: temporal drift modeling
                # Gate modulates token features based on time information
                gate = self.temporal_gate(time_emb)  # (B, L, D)
