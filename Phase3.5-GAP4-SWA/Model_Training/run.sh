...
# Phase3.5-GAP4-SWA: Multi-checkpoint saving + post-hoc selection
# This experiment saves the last N checkpoints (default 5) to enable
# post-hoc selection based on evaluation results.
#
# Only change vs T2 baseline:
#   save_last_n: 5 (save last 5 checkpoints instead of only best AUC)
#
...
"${PYTHON_BIN_CMD[@]}" -u "${SCRIPT_DIR}/train.py" \
    --run_name "${RUN_NAME:-phase3_5_gap4_multi_checkpoint}" \
