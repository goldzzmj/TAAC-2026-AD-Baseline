...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        save_last_n: Optional[int] = None,  # Save last N checkpoints (0 = disable)
        ...
        # SWA / Multi-checkpoint: Save last N checkpoints for post-hoc selection
        self.save_last_n = save_last_n if save_last_n is not None else 0
        self.saved_checkpoints = []  # List of (epoch, step, auc, logloss, checkpoint_dir) tuples
        self.ckpt_summary_path = os.path.join(self.save_dir, 'checkpoint_summary.json')
    ...
    def _write_checkpoint_summary(self) -> None:
        """Write checkpoint summary to JSON file.

        This file contains information about all saved checkpoints,
        allowing post-hoc selection based on evaluation results.
        """
        ...
        summary = {
            ...
            "best_by_auc": max(self.saved_checkpoints, key=lambda x: x["val_auc"]),
            "best_by_logloss": min(self.saved_checkpoints, key=lambda x: x["val_logloss"]),
        }
        self._write_json(self.ckpt_summary_path, summary)
        logging.info(f"Checkpoint summary saved to {self.ckpt_summary_path}")
    ...
    def _save_last_n_checkpoints(
        ...
        """Save last N checkpoints for post-hoc selection.

        This saves checkpoints from the last N epochs, allowing evaluation
        and selection of the best checkpoint based on Eval AUC after training.

        Args:
            epoch: Current epoch number
            total_step: Total training steps so far
            val_auc: Validation AUC
            val_logloss: Validation LogLoss
        """
        if self.save_last_n <= 0:
            return

        # Create checkpoint directory
        ckpt_dir = os.path.join(
            self.save_dir,
            self._build_step_dir_name(total_step, is_best=False),
        )

        # Save checkpoint
        self._save_step_checkpoint(total_step, is_best=False, skip_model_file=False)
        ...
        if len(self.saved_checkpoints) > self.save_last_n:
            # Remove oldest checkpoint
            old_ckpt = self.saved_checkpoints.pop(0)
            old_dir = old_ckpt["checkpoint_dir"]
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            # Save last N checkpoints for post-hoc selection
            self._save_last_n_checkpoints(epoch, total_step, val_auc, val_logloss)
            metric_record = {
                "epoch": epoch,
                "global_step": total_step,
