...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        seq_c_fid47_removed: bool = False,
    ) -> None:
        """
        Args:
            ...
            row_group_range: ``(start, end)`` slice of Row Groups; ``None`` to
                use all Row Groups.
            row_group_list: Phase0 新增, 直接传入预排序的 Row Group 列表,
                用于时间切分验证。优先级高于 row_group_range。
            ...
            seq_c_fid47_removed: if True, remove fid47 from seq_c sideinfo features.
        """
        super().__init__()
        ...
        self.seq_c_fid47_removed = seq_c_fid47_removed
        ...
        # Phase0: row_group_list 优先于 row_group_range
        ...
        # SEQ-1: Remove seq_c fid47 when seq_c_fid47_removed is set
        if self.seq_c_fid47_removed and 'seq_c' in self.seq_domains:
            if 47 in self.sideinfo_fids['seq_c']:
                self.sideinfo_fids['seq_c'] = [fid for fid in self.sideinfo_fids['seq_c'] if fid != 47]
                self.seq_domain_vocab_sizes['seq_c'] = [vs for fid, vs in
                    zip(self.sideinfo_fids['seq_c'], self.seq_domain_vocab_sizes['seq_c'])]
                # Update seq_c buffer with new feature count
                domain = 'seq_c'
                n_feats = len(self.sideinfo_fids[domain])
                self._buf_seq[domain] = np.zeros((B, n_feats, self._seq_maxlen[domain]), dtype=np.int64)
                logging.info("SEQ-1: Removed fid47 from seq_c sideinfo features")
        ...
        logging.info(
            ...
            f"seq_c_fid47_removed={seq_c_fid47_removed}")
...
def _get_rg_timestamp_stats(pq_files):
    """Phase0 新增: 获取每个 Row Group 的 timestamp 统计信息。

    利用 Parquet 元数据中的 min/max 统计获取每个 Row Group 的
    timestamp 最大值（无需读取实际数据，非常高效）。

    Returns:
        rg_timestamps: list of (max_timestamp,) per row group,
                       与 rg_info 列表一一对应。
    """
    rg_timestamps = []
    for f in pq_files:
        ...
        if ts_col_idx is None:
            logging.warning("timestamp column not found in %s, falling back to file order", f)
            for i in range(pf.metadata.num_row_groups):
                rg_timestamps.append((0,))
            continue
        for i in range(pf.metadata.num_row_groups):
            ...
            if col_meta.statistics is not None and col_meta.statistics.has_min_max:
                max_ts = col_meta.statistics.max
                rg_timestamps.append((max_ts,))
            else:
                # 无统计信息时读取首行 timestamp 作为近似值
                try:
                    batch = next(pf.iter_batches(batch_size=1, row_groups=[i], columns=['timestamp']))
                    ts_val = batch.column(0)[0].as_py()
                    rg_timestamps.append((ts_val if ts_val else 0,))
                except Exception:
                    rg_timestamps.append((0,))
    return rg_timestamps
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    Phase0 改动: 新增 ``time_split`` 参数 (默认 True).

    当 ``time_split=True`` 时，验证集选取 timestamp 最大的 Row Groups
    (时间最近的交互)，而非简单的文件尾部 Row Groups。这样验证集更接近
    线上分布，valid AUC 与 inference AUC 的 gap 应该更小。

    当 ``time_split=False`` 时，回退到原始行为 (按文件顺序尾部切分)。

    Returns:
        A tuple ``(train_loader, valid_loader, train_dataset)``.
    """
    random.seed(seed)
    ...
    if os.path.isdir(data_dir):
        ...
        raise FileNotFoundError(f"Parquet path does not exist: {data_dir}")
    ...
    if total_rgs == 1:
        logging.warning(
            "Only one Row Group was found. Reusing it for both training and "
            "validation so the local pipeline can run end-to-end."
        )
        train_row_group_range = (0, 1)
        valid_row_group_range = (0, 1)
        train_rows = rg_info[0][2]
        valid_rows = rg_info[0][2]
    else:
        if valid_ratio <= 0:
            n_valid_rgs = 1
        else:
        ...
        if time_split:
            # Phase0: 按 timestamp 排序 Row Groups，取最近时间的作为验证集
            rg_timestamps = _get_rg_timestamp_stats(pq_files)
            # 按最大 timestamp 排序 (升序)，得到排序后的索引
            sorted_indices = sorted(range(total_rgs), key=lambda idx: rg_timestamps[idx][0])
            # sorted_indices[:n_train_rgs] = 时间较早的训练集
            # sorted_indices[n_train_rgs:] = 时间较晚的验证集
            train_indices = set(sorted_indices[:n_train_rgs])
            valid_indices = sorted(sorted_indices[n_train_rgs:])  # 保持顺序

            logging.info(
                "Phase0 time-split: sorted %d Row Groups by timestamp, "
                "train=%d (earlier), valid=%d (later)",
                total_rgs, n_train_rgs, n_valid_rgs,
            )
            if rg_timestamps:
                train_ts_range = [rg_timestamps[sorted_indices[0]][0],
                                  rg_timestamps[sorted_indices[n_train_rgs - 1]][0]]
                valid_ts_range = [rg_timestamps[sorted_indices[n_train_rgs]][0],
                                  rg_timestamps[sorted_indices[-1]][0]]
                logging.info("  Train timestamp range: %s ~ %s", train_ts_range[0], train_ts_range[1])
                logging.info("  Valid timestamp range: %s ~ %s", valid_ts_range[0], valid_ts_range[1])
        else:
            # 原始行为: 按文件顺序尾部切分
            train_indices = None
            valid_indices = None
        ...
        if time_split:
            # 构建 sorted rg_info 用于范围映射
            sorted_rg = [rg_info[i] for i in sorted_indices]
            # 为 PCVRParquetDataset 构建需要的信息
            # 由于 row_group_range 是基于原始 _rg_list 的切片索引,
            # 但 time_split 需要选择不连续的 row groups，
            # 我们需要用另一种方式传递。
            # 方法: 用排序后的 rg_info 构建虚拟范围，通过修改 rg_list 顺序实现
            train_rg_list = [rg_info[i] for i in sorted_indices[:n_train_rgs]]
            valid_rg_list = [rg_info[i] for i in sorted_indices[n_train_rgs:]]
    ...
    logging.info(
        "Row Group split: train=%s (%s rows), valid=%s (%s rows), time_split=%s",
        train_row_group_range if not time_split else f"sorted_{n_train_rgs}",
        train_rows,
        valid_row_group_range if not time_split else f"sorted_{n_valid_rgs}",
        valid_rows,
        time_split,
    )

    # Phase0: time_split 模式下需要直接传入 rg_list 而非 range
    if time_split and total_rgs > 1:
        train_dataset = PCVRParquetDataset(
            ...
            row_group_list=train_rg_list,
            clip_vocab=clip_vocab,
            seq_c_fid47_removed=seq_c_fid47_removed,
        )
    ...
    logging.info(f"Parquet train: {train_rows} rows, valid: {valid_rows} rows, "
                 ...
                 f"time_split={time_split}, seq_c_fid47_removed={seq_c_fid47_removed}")
