...
class RankMixerNSTokenizer(nn.Module):
    ...
    def forward(self, int_feats: torch.Tensor) -> torch.Tensor:
        ...
        for chunk, proj in zip(chunks, self.token_projs):
            tokens.append(F.silu(proj(chunk.contiguous())).unsqueeze(1))  # (B, 1, d_model)
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        use_deep_classifier: bool = False,
    ) -> None:
        super().__init__()
        ...
        if use_deep_classifier:
            # 3-layer classifier with expansion bottleneck
            # 64 → 128 → 64 → 1 (adds ~16.5K params)
            hidden = d_model * 2
