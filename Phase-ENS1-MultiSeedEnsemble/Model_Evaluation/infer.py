...
_FALLBACK_MODEL_CFG = {
    ...
    'use_shared_fid_joint': False,
    'use_target_aware_query': False,
}
...
def resolve_eval_batch_size(train_config: Dict[str, Any]) -> Tuple[int, int]:
    ...
    Default to the training batch size for maximum compatibility with the
    checkpoint's recorded runtime config. EVAL_BATCH_SIZE / INFER_BATCH_SIZE
    can still override it when the platform accepts larger inference batches.
    """
    train_bs = int(train_config.get('batch_size', _FALLBACK_BATCH_SIZE))
    eval_bs = _env_int('EVAL_BATCH_SIZE', _env_int('INFER_BATCH_SIZE', train_bs))
    min_bs = _env_int('EVAL_MIN_BATCH_SIZE', min(train_bs, eval_bs))
...
def build_model(
    ...
    # Resolve shared-fid joint modeling offsets from dataset schema.
    shared_fid_dense_offsets = None
    shared_fid_int_indices = None
    if model_cfg.get('use_shared_fid_joint', False):
        ...
        # Int indices from user_int_schema entries (feature_specs has vocab_size, not fid)
        int_fid_to_idx = {fid: i for i, (fid, _, _) in enumerate(dataset.user_int_schema.entries)}
        ...
        if dense_offsets and int_indices:
            ...
            logging.info(
                f"Shared-FID joint eval: {len(dense_offsets)} shared fids"
            )
...
def load_ema_weights(model: nn.Module, model_dir: str, device: str) -> bool:
    """Load EMA shadow weights into the model if model_ema.pt exists.

    EMA checkpoint format: {'model_state': ..., 'ema_state': {'shadow': {name: tensor}}}
    """
    # Try multiple EMA file locations
    for ema_name in ['best_model_ema.pt', 'model_ema.pt', 'latest_model_ema.pt']:
        ema_path = os.path.join(model_dir, ema_name)
        if os.path.exists(ema_path):
            try:
                ema_ckpt = torch.load(ema_path, map_location=device, weights_only=True)
                ema_shadow = ema_ckpt.get('ema_state', {}).get('shadow', {})
                if ema_shadow:
                    loaded = 0
                    for name, param in model.named_parameters():
                        if name in ema_shadow:
                            ...
                            loaded += 1
                    logging.info(f"Loaded EMA weights from {ema_path} ({loaded} params)")
                    return True
            except Exception as exc:
                logging.warning(f"Failed to load EMA from {ema_path}: {exc}")
    return False


def build_and_load_single_model(
    dataset: PCVRParquetDataset,
    model_cfg: Dict[str, Any],
    ns_groups_json: Optional[str],
    device: str,
    ckpt_path: str,
    model_dir: str,
) -> nn.Module:
    """Build a single model, load weights, apply EMA, return in eval mode."""
    model = build_model(dataset, model_cfg, ns_groups_json, device)
    load_model_state_strict(model, ckpt_path, device)
    load_ema_weights(model, model_dir, device)
    model.eval()
    return model
...
def run_ensemble_inference(
    ...
    """Run inference with multiple models and average predictions."""
    test_dataset = make_test_dataset(data_dir, schema_path, batch_size, seq_max_lens)
    test_loader = make_test_loader(test_dataset, num_workers, prefetch_factor)

    all_user_ids: List[Any] = []
    all_probs_accum: List[List[float]] = []  # [model_idx][sample_idx]
    for _ in models:
        all_probs_accum.append([])
    ...
    with torch.inference_mode():
        for batch_idx, batch in enumerate(test_loader):
            ...
            for m_idx, mdl in enumerate(models):
                logits, _ = mdl.predict(model_input)
                probs = torch.sigmoid(logits.squeeze(-1)).float().cpu().tolist()
                all_probs_accum[m_idx].extend(probs)
            ...
            if (batch_idx + 1) % log_every == 0:
                elapsed = max(time.time() - start_time, 1e-6)
                logging.info(
                    "  [Ensemble x%d] Processed %d samples (%.1f samples/s)",
                    len(models), raw_count, raw_count / elapsed)

    # Average predictions across models
    avg_probs = np.mean(np.array(all_probs_accum), axis=0).tolist()

    predictions_dict = dict(zip(all_user_ids, avg_probs))

    elapsed = max(time.time() - start_time, 1e-6)
    logging.info(
        "Ensemble inference complete: %d models, %d rows, %.1f samples/s",
        len(models), raw_count, raw_count / elapsed)

    return predictions_dict, raw_count
...
def main() -> None:
    ...
    # EMA: prefer EMA weights when available
    loaded_ema = load_ema_weights(model, model_dir, device)
    if loaded_ema:
        logging.info("Using EMA model weights for inference")
    ...
    ensemble_model_files: List[str] = []
    for suffix in ['_seed2', '_seed3', '_seed2024', '_seed2025', '_v2', '_v3']:
        for prefix in ['best_model', 'model', 'latest_model']:
            candidate = os.path.join(model_dir, f'{prefix}{suffix}.pt')
            if os.path.exists(candidate):
                ensemble_model_files.append(candidate)
                logging.info(f"Found ensemble model: {candidate}")
                break
    ...
    while True:
        try:
            if ensemble_model_files:
                logging.info(
                    "Ensemble mode: %d models", 1 + len(ensemble_model_files))
                # Build primary + ensemble models
                all_models: List[nn.Module] = [model]  # primary model already built
                for ens_path in ensemble_model_files:
                    try:
                        ens_model = build_and_load_single_model(
                            metadata_dataset, model_cfg, ns_groups_json, device,
                            ens_path, model_dir)
                        ens_model = maybe_compile_model(ens_model)
                        ens_model.eval()
                        all_models.append(ens_model)
                    except Exception as exc:
                        logging.warning(
                            "Skipping ensemble model %s: %s", ens_path, exc)

                predictions_dict, raw_count = run_ensemble_inference(
                    models=all_models,
                    data_dir=data_dir,
                    schema_path=schema_path,
                    seq_max_lens=seq_max_lens,
