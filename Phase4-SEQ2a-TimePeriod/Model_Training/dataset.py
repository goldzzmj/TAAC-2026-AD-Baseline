...
class PCVRParquetDataset(IterableDataset):
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        # ---- Time period features (sinusoidal encoding) ----
        ts_float = timestamps.astype(np.float32)
        hour_of_day = (ts_float / 3600.0) % 24.0
        day_of_week = (ts_float / 86400.0) % 7.0
        hour_sin = np.sin(2.0 * np.pi * hour_of_day / 24.0)
        hour_cos = np.cos(2.0 * np.pi * hour_of_day / 24.0)
        day_sin = np.sin(2.0 * np.pi * day_of_week / 7.0)
        day_cos = np.cos(2.0 * np.pi * day_of_week / 7.0)
        time_period = np.stack([hour_sin, hour_cos, day_sin, day_cos], axis=-1)
        result['time_period'] = torch.from_numpy(time_period.astype(np.float32))
