...
class ModelInput(NamedTuple):
    ...
    time_period: Optional[torch.Tensor] = None  # (B, 4) sinusoidal time features
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # Time period features (gated additive residual)
        self.use_time_period = use_time_period
        if use_time_period:
            self.time_period_proj = nn.Sequential(
                nn.Linear(4, d_model),
                nn.LayerNorm(d_model),
                nn.SiLU(),
            )
            self.time_period_norm = nn.LayerNorm(d_model)
            self.time_period_gate = nn.Parameter(torch.tensor(gate_logit_init))
    ...
    def _init_params(self) -> None:
        ...
        if hasattr(self, 'time_period_gate'):
            self.time_period_gate.data.clamp_(min=-4.0, max=4.0)
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # 5. Time period fusion (gated additive residual)
        if self.use_time_period and inputs.time_period is not None:
            gate = torch.sigmoid(self.time_period_gate)
            output = self.time_period_norm(output + gate * self.time_period_proj(inputs.time_period))
    ...
    def predict(self, inputs: ModelInput) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Time period fusion (gated additive residual)
        if self.use_time_period and inputs.time_period is not None:
            gate = torch.sigmoid(self.time_period_gate)
            output = self.time_period_norm(output + gate * self.time_period_proj(inputs.time_period))
