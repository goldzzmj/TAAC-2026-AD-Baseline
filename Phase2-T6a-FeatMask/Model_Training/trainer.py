...
class PCVRHyFormerRankingTrainer:
    ...
    def _apply_feature_masking(self, device_batch: Dict[str, Any]) -> Dict[str, Any]:
        """Random feature masking on dense features for data augmentation."""
        if self.feature_mask_ratio <= 0:
            return device_batch

        masked = dict(device_batch)
        for key in ('user_dense_feats', 'item_dense_feats'):
            if key in masked and isinstance(masked[key], torch.Tensor):
                mask = torch.rand_like(masked[key]) > self.feature_mask_ratio
                masked[key] = masked[key] * mask.float()
        return masked

    def _train_step(self, batch: Dict[str, Any]) -> float:
        """Run a single training step and return the scalar loss value."""
        device_batch = self._batch_to_device(batch)
        device_batch = self._apply_feature_masking(device_batch)
