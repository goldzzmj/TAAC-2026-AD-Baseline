...
class DomainReliabilityGate(nn.Module):
    """Domain-level dynamic reliability gating module.

    Computes per-domain reliability scores based on the interaction between
    pooled sequence representations and a context vector, then applies
    multiplicative gating to scale each domain's sequence tokens.
    """
    ...
    def forward(self, seq_tokens_list, seq_masks_list, context):
        """Applies domain-level reliability gating.

        Args:
            seq_tokens_list: List of (B, L_i, D) tensors, length num_domains.
            seq_masks_list: List of (B, L_i) masks, True = padding.
            context: (B, D) context tensor.

        Returns:
            List of gated (B, L_i, D) tensors, length num_domains.
        """
        ctx_q = self.context_proj(context)
        gated_list = []
        for i in range(self.num_domains):
            valid_mask = (1.0 - seq_masks_list[i].float()).unsqueeze(-1)
            seq_sum = (seq_tokens_list[i] * valid_mask).sum(dim=1)
            seq_count = valid_mask.sum(dim=1).clamp(min=1)
            pooled = seq_sum / seq_count
            domain_key = self.domain_proj(pooled)
            score = (domain_key * ctx_q).sum(dim=-1) + self.gate_bias[i]
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        if use_shared_fid_joint and self.has_user_dense and shared_fid_dense_offsets and shared_fid_int_indices:
            ...
            self._shared_fid_dense_offsets = shared_fid_dense_offsets
            self._shared_fid_int_indices = shared_fid_int_indices
        ...
        # Domain-level Dynamic Reliability Gate (DOM-1)
        if use_domain_gate:
    ...
    def _compute_shared_fid_cross(
        ...
        for i, (dense_off, dense_len) in enumerate(self._shared_fid_dense_offsets):
            ...
            if int_len == 1:
                ...
                mask = (vals != 0).float().unsqueeze(-1)
                count = mask.sum(dim=1).clamp(min=1)
                int_emb = (emb_all * mask).sum(dim=1) / count

            if dense_len == 1:
                dense_val = user_dense_feats[:, dense_off].unsqueeze(-1)
            else:
                dense_val = user_dense_feats[:, dense_off:dense_off + dense_len].mean(dim=-1, keepdim=True)

            cross = int_emb * dense_val
            cross_parts.append(cross)

        cat_cross = torch.cat(cross_parts, dim=-1)
        return F.silu(self._shared_fid_joint_proj(cat_cross))
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # 4. Compute item_ctx for domain gate (if enabled)
        if self.use_domain_gate:
            item_ctx = torch.cat(item_parts, dim=1).mean(dim=1)  # (B, D)

        # 5. Dropout + Domain Gate + MultiSeqHyFormerBlock stack + output projection
        output = self._run_multi_seq_blocks(
            q_tokens_list, ns_tokens, seq_tokens_list, seq_masks_list,
            apply_dropout=self.training, context=item_ctx
        )
    ...
    def predict(self, inputs: ModelInput) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Compute item_ctx for domain gate (if enabled)
        if self.use_domain_gate:
            item_ctx = torch.cat(item_parts, dim=1).mean(dim=1)  # (B, D)

        output = self._run_multi_seq_blocks(
            q_tokens_list, ns_tokens, seq_tokens_list, seq_masks_list,
            apply_dropout=False, context=item_ctx
        )
        output = self._apply_second_pass_fusion(
            output, torch.cat(user_parts, dim=1), torch.cat(item_parts, dim=1),
            seq_tokens_list, seq_masks_list)
