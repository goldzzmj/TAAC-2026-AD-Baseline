...
class ExponentialMovingAverage:
    """维护模型参数的指数移动平均（EMA）影子副本。

    训练时使用原始模型参数更新，验证/推理时使用 EMA 参数。
    这能显著减小训练-测试 AUC 间隙，提升泛化能力。
    """
    ...
    def _get_decay(self) -> float:
        """使用 warmup 平滑 EMA decay。"""
        if self.step_count < self.warmup_steps:
            return self.decay * float(self.step_count) / max(1, self.warmup_steps)
        return self.decay

    def update(self) -> None:
        """更新 EMA 影子参数。在每次 optimizer.step() 之后调用。"""
        self.step_count += 1
...
def _hash_ids(ids: torch.Tensor, hash_size: int) -> torch.Tensor:
    """Map positive ids into ``[1, hash_size]`` while preserving padding 0."""
    if hash_size <= 0:
        return ids
    positive_hash = ((ids.clamp_min(1) - 1) % hash_size) + 1
    return torch.where(ids > 0, positive_hash, torch.zeros_like(ids))
...
class DomainReliabilityGate(nn.Module):
    """Domain-level reliability gating.

    Computes per-domain gating weights based on the interaction between
    each domain's pooled representation and the item context (target).
    This allows the model to dynamically emphasize more reliable behavior
    domains for each prediction.

    Gate initialization: zero-init bias so sigmoid(bias) ~ 0.5 -> scale = 1.0 (identity).
    """

    def __init__(self, d_model: int, num_domains: int) -> None:
        super().__init__()
        self.num_domains = num_domains
        self.domain_proj = nn.Linear(d_model, d_model, bias=False)
        self.context_proj = nn.Linear(d_model, d_model, bias=False)
        self.gate_bias = nn.Parameter(torch.zeros(num_domains))

    def forward(
        self,
        seq_tokens_list: list,     # list of (B, L_i, D)
        seq_masks_list: list,       # list of (B, L_i), True = padding
        context: torch.Tensor,      # (B, D) item context
    ) -> list:
        ctx_q = self.context_proj(context)  # (B, D)
        gated_list = []
        for i in range(self.num_domains):
            # Mean pool domain tokens (respecting padding mask)
            valid_mask = (1.0 - seq_masks_list[i].float()).unsqueeze(-1)  # (B, L_i, 1)
            seq_sum = (seq_tokens_list[i] * valid_mask).sum(dim=1)   # (B, D)
            seq_count = valid_mask.sum(dim=1).clamp(min=1)           # (B, 1)
            pooled = seq_sum / seq_count                              # (B, D)
            # Compute gate score via dot-product attention
            domain_key = self.domain_proj(pooled)                     # (B, D)
            score = (domain_key * ctx_q).sum(dim=-1) + self.gate_bias[i]  # (B,)
            gate = torch.sigmoid(score)  # (B,)
            # Apply: scale = 2 * gate, init at 1.0 (identity)
            gated_list.append(seq_tokens_list[i] * (2.0 * gate.unsqueeze(1).unsqueeze(2)))
        return gated_list


class DINActivationUnit(nn.Module):
    """Lightweight DIN-style activation unit for target-conditioned pooling.

    Computes attention weights over sequence tokens conditioned on a target
    representation, then produces a weighted sum (target-aware pooling).

    Unlike standard mean pooling, this allows the query generator to focus
    on sequence items most relevant to the current target item.
    """

    def __init__(self, d_model: int) -> None:
        super().__init__()
        # DIN activation: concat(seq, target, seq-target, seq*target) -> scalar weight
    ...
    def forward(
        ...
        target_expanded = target.unsqueeze(1).expand(B, L, D)

        # Interaction features: [seq, target, seq-target, seq*target]
        act_input = torch.cat([
            seq_tokens,
            target_expanded,
        ...
        # Compute attention weight per position
        attn = self.activation(act_input)  # (B, L, 1)

        # Mask out padding positions
        attn = attn.masked_fill(padding_mask.unsqueeze(-1), float('-inf'))
        attn = F.softmax(attn, dim=1)  # (B, L, 1)

        # Handle all-padding case (all -inf -> nan after softmax)
        attn = attn.nan_to_num(nan=0.0)

        # Weighted sum
        pooled = (attn * seq_tokens).sum(dim=1)  # (B, D)
        return pooled
...
class MultiSeqQueryGenerator(nn.Module):
    ...
    def __init__(
        ...
        num_sequences: int,
        hidden_mult: int = 4,
        use_target_aware_query: bool = False,
    ) -> None:
        super().__init__()
        self.num_queries = num_queries
        self.num_sequences = num_sequences
        self.d_model = d_model
        self.use_target_aware_query = use_target_aware_query
        ...
        # Target-aware query: DIN activation units for target-conditioned pooling
        if use_target_aware_query:
            self.din_activations = nn.ModuleList([
                DINActivationUnit(d_model) for _ in range(num_sequences)
            ])
    ...
    def forward(
        ...
        item_context: torch.Tensor = None,  # (B, D) optional target item representation
    ) -> list:
        ...
        Args:
            ...
            seq_padding_masks: List of (B, L_i) masks, length S. True
                indicates padding.
            item_context: (B, D) optional target item representation for
                DIN-style target-conditioned pooling.
        ...
        for i in range(self.num_sequences):
            if self.use_target_aware_query and item_context is not None:
                # DIN-style target-conditioned pooling
                seq_pooled = self.din_activations[i](
                    seq_tokens_list[i], item_context, seq_padding_masks[i])
            else:
                # Fallback: MeanPool(Seq_i)
...
class GroupNSTokenizer(nn.Module):
    ...
    def __init__(self, feature_specs: List[Tuple[int, int, int]],
                 ...
                 emb_hash_size: int = 0) -> None:
        super().__init__()
        self.feature_specs = feature_specs
        self.groups = groups
        self.emb_dim = emb_dim
        self.emb_skip_threshold = emb_skip_threshold
        self.emb_hash_size = emb_hash_size

        # One embedding table per fid. Ultra-high-cardinality features can be
        # hashed into a bounded table instead of being represented as zeros.
        embs = []
        hash_sizes = []
        for vs, offset, length in feature_specs:
            ...
            use_hash = (
                ...
                and vs_int > emb_skip_threshold
                and emb_hash_size > 0
            )
            skip = vs_int <= 0 or (
                emb_skip_threshold > 0 and vs_int > emb_skip_threshold and not use_hash
            )
            if skip:
                embs.append(None)
                hash_sizes.append(0)
            else:
                table_size = int(emb_hash_size) if use_hash else vs_int
                embs.append(nn.Embedding(table_size + 1, emb_dim, padding_idx=0))
                hash_sizes.append(table_size if use_hash else 0)
        self.embs = nn.ModuleList([e for e in embs if e is not None])
        self._emb_hash_sizes = hash_sizes
            ...
            for fid_idx in group:
                ...
                if emb_real_idx == -1:
                    ...
                    if length == 1:
                        ...
                        vals = _hash_ids(vals, self._emb_hash_sizes[fid_idx])
                        fid_emb = emb_layer(vals)  # (B, emb_dim)
                    else:
...
class RankMixerNSTokenizer(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            emb_hash_size: Hash-table size for skipped high-cardinality features.
        """
        super().__init__()
        self.feature_specs = feature_specs
        self.groups = groups
...
class TargetAttentionPooling(nn.Module):
    """DIN-style target-aware pooling over each behavior sequence.

    The target representation is derived from item-side non-sequence tokens,
    then used to activate only the history tokens that are relevant to the
    candidate item. This is complementary to the existing HyFormer query path:
    it adds a direct target-conditioned interest vector without changing the
    input data format.
    """
    ...
    def forward(
        ...
        pooled = []
        for seq_tokens, padding_mask in zip(seq_tokens_list, seq_masks_list):
            target = target_token.unsqueeze(1).expand_as(seq_tokens)
            act_in = torch.cat([
                seq_tokens,
                target,
                seq_tokens - target,
            ...
            scores = self.local_activation(act_in).squeeze(-1)
            ...
            pooled.append((seq_tokens * weights.unsqueeze(-1)).sum(dim=1))

        return self.out_proj(torch.cat(pooled, dim=-1))


class LowRankCrossFusion(nn.Module):
    """Small DCN-V2-style low-rank cross network over the final representation."""
    ...
    def __init__(
        ...
        self.u_layers = nn.ModuleList([
            nn.Linear(d_model, rank, bias=False) for _ in range(num_layers)
        ])
        self.v_layers = nn.ModuleList([
            nn.Linear(rank, d_model) for _ in range(num_layers)
        ])
    ...
    def forward(self, x0: torch.Tensor) -> torch.Tensor:
        x = x0
        for u, v, norm in zip(self.u_layers, self.v_layers, self.norms):
            crossed = v(F.gelu(u(x)))
            x = norm(x + x0 * self.dropout(crossed))
        return x


class FinalContextFusion(nn.Module):
    """Lightweight user/item context residual over the final HyFormer vector."""
    ...
    def __init__(
        ...
        self.mlp = nn.Sequential(
            nn.Linear(5 * d_model, hidden_dim),
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # Conservative second-pass model enhancements
        use_target_attention: bool = True,
        target_attention_hidden_mult: int = 2,
        use_cross_fusion: bool = True,
        cross_fusion_layers: int = 1,
        cross_fusion_rank: int = 16,
        use_context_fusion: bool = True,
        context_fusion_hidden_mult: int = 2,
        fusion_gate_init: float = 0.1,
        # Shared-FID joint modeling
        use_shared_fid_joint: bool = False,
        shared_fid_dense_offsets: Optional[List[Tuple[int, int]]] = None,
        shared_fid_int_indices: Optional[List[int]] = None,
        # Target-aware query (QRY-1)
        use_target_aware_query: bool = False,
        # Domain-level dynamic reliability gating (DOM-1)
        ...
        self.use_shared_fid_joint = use_shared_fid_joint
        self.use_target_aware_query = use_target_aware_query
        self.use_domain_gate = use_domain_gate
        gate_init = min(max(float(fusion_gate_init), 1e-4), 1.0 - 1e-4)
        gate_logit_init = math.log(gate_init / (1.0 - gate_init))
        ...
        # Shared-FID joint modeling: cross int embeddings with dense values
        # For shared-fid features (62,63,64,65,66,89,90,91), their int embedding
        # and dense value jointly describe the same underlying signal.
        # This module applies element-wise multiplication: int_emb * dense_val,
        # then projects the result to enhance the user_dense token.
        self._shared_fid_joint_proj = None
        self._shared_fid_dense_offsets = None
        self._shared_fid_int_indices = None
        if use_shared_fid_joint and self.has_user_dense and shared_fid_dense_offsets and shared_fid_int_indices:
            num_shared = len(shared_fid_dense_offsets)
            # Each shared-fid contributes emb_dim from int embedding cross dense value
            self._shared_fid_dense_offsets = shared_fid_dense_offsets  # [(offset, length), ...]
            self._shared_fid_int_indices = shared_fid_int_indices      # [fid_idx_in_feature_specs, ...]
            self._shared_fid_joint_proj = nn.Sequential(
                nn.Linear(num_shared * emb_dim, d_model),
                nn.LayerNorm(d_model),
            )
            self._shared_fid_gate = nn.Parameter(torch.zeros(1))
            logging.info(
                f"Shared-FID joint modeling enabled: {num_shared} shared fids, "
                f"dense_offsets={shared_fid_dense_offsets}, "
                f"int_indices={shared_fid_int_indices}"
            )
        ...
        def _make_seq_embs(vocab_sizes):
            ...
            for vs in vocab_sizes:
                ...
                if skip:
                    ...
                    table_size = int(emb_hash_size) if use_hash else vs_int
                    embs_raw.append(nn.Embedding(table_size + 1, emb_dim, padding_idx=0))
                    hash_sizes.append(table_size if use_hash else 0)
            ...
            return module_list, index_map, is_id, hash_sizes

        # ================== Dynamic Sequence Embeddings ==================
        self._seq_embs = nn.ModuleDict()
        self._seq_emb_index = {}    # domain -> index_map
        self._seq_emb_hash_sizes = {}  # domain -> hash size per feature
        ...
        for domain in self.seq_domains:
            ...
            embs, idx_map, is_id, hash_sizes = _make_seq_embs(vs)
            self._seq_embs[domain] = embs
            self._seq_emb_index[domain] = idx_map
            self._seq_emb_hash_sizes[domain] = hash_sizes
        ...
        self.query_generator = MultiSeqQueryGenerator(
            ...
            hidden_mult=hidden_mult,
            use_target_aware_query=use_target_aware_query,
        )

        # Domain-level reliability gate (DOM-1)
        if use_domain_gate:
            self.domain_gate = DomainReliabilityGate(d_model, self.num_sequences)
        ...
        if use_target_attention:
            self.target_attention = TargetAttentionPooling(
                d_model=d_model,
                num_sequences=self.num_sequences,
                hidden_mult=target_attention_hidden_mult,
                dropout=dropout_rate,
            )
            self.target_fusion_norm = nn.LayerNorm(d_model)
            self.target_fusion_gate = nn.Parameter(
                torch.tensor(gate_logit_init))

        if use_cross_fusion:
            self.cross_fusion = LowRankCrossFusion(
                d_model=d_model,
                num_layers=max(1, int(cross_fusion_layers)),
                rank=max(1, int(cross_fusion_rank)),
                dropout=dropout_rate,
            )
            self.cross_fusion_norm = nn.LayerNorm(d_model)
            self.cross_fusion_gate = nn.Parameter(
                torch.tensor(gate_logit_init))

        if use_context_fusion:
            self.context_fusion = FinalContextFusion(
                d_model=d_model,
                hidden_mult=max(1, int(context_fusion_hidden_mult)),
                dropout=dropout_rate,
            )
            self.context_fusion_norm = nn.LayerNorm(d_model)
            self.context_fusion_gate = nn.Parameter(
                torch.tensor(gate_logit_init))
        ...
        if emb_skip_threshold > 0:
            def _count_filtered_hashed(vocab_sizes, emb_index, hash_sizes):
                filtered = sum(1 for idx in emb_index if idx == -1)
                hashed = sum(1 for h in hash_sizes if h > 0)
                return filtered, hashed, len(vocab_sizes)
            for domain in self.seq_domains:
                f, h, t = _count_filtered_hashed(
                    self._seq_vocab_sizes[domain],
                    self._seq_emb_index[domain],
                    self._seq_emb_hash_sizes[domain],
                )
                if f > 0 or h > 0:
                    logging.info(
                        f"emb_skip_threshold={emb_skip_threshold}, emb_hash_size={emb_hash_size}: "
                        f"{domain} skipped {f}/{t}, hashed {h}/{t} features")
            for name, tokenizer in [
                ("user_ns", self.user_ns_tokenizer),
                ("item_ns", self.item_ns_tokenizer),
            ]:
                f = sum(1 for idx in tokenizer._emb_index if idx == -1)
                h = sum(1 for size in tokenizer._emb_hash_sizes if size > 0)
                t = len(tokenizer._emb_index)
                if f > 0 or h > 0:
                    logging.info(
                        f"emb_skip_threshold={emb_skip_threshold}, emb_hash_size={emb_hash_size}: "
                        f"{name} skipped {f}/{t}, hashed {h}/{t} features")
    ...
    def _init_params(self) -> None:
        ...
        if hasattr(self, 'target_fusion_gate'):
            self.target_fusion_gate.data.clamp_(min=-4.0, max=4.0)
        if hasattr(self, 'cross_fusion_gate'):
            self.cross_fusion_gate.data.clamp_(min=-4.0, max=4.0)
        if hasattr(self, 'context_fusion_gate'):
            self.context_fusion_gate.data.clamp_(min=-4.0, max=4.0)
    ...
    def reinit_high_cardinality_params(
        ...
        if cardinality_threshold < 0:
            logging.info("High-cardinality Embedding reinitialization disabled "
    ...
    def _compute_shared_fid_cross(
        self,
        user_int_feats: torch.Tensor,
        user_dense_feats: torch.Tensor,
        user_ns_tokenizer: nn.Module,
    ) -> torch.Tensor:
        """Compute shared-fid joint cross features: int_emb * dense_val.

        For each shared-fid, extracts the int embedding (from the NS tokenizer's
        embedding table) and the dense value (from user_dense_feats), computes
        element-wise multiplication, and projects the concatenated result.

        Returns:
            Tensor of shape (B, d_model) representing the cross signal.
        """
        cross_parts = []
        for i, (dense_off, dense_len) in enumerate(self._shared_fid_dense_offsets):
            fid_idx = self._shared_fid_int_indices[i]
            # Get int embedding for this fid from the user NS tokenizer
            vs, int_off, int_len = user_ns_tokenizer.feature_specs[fid_idx]
            emb_real_idx = user_ns_tokenizer._emb_index[fid_idx]

            if emb_real_idx == -1 or dense_len == 0:
                # Skipped or missing: zero vector
                cross_parts.append(user_int_feats.new_zeros(user_int_feats.shape[0], user_ns_tokenizer.emb_dim))
                continue

            emb_layer = user_ns_tokenizer.embs[emb_real_idx]

            # Int embedding lookup
            if int_len == 1:
                vals = user_int_feats[:, int_off].long()
                vals = _hash_ids(vals, user_ns_tokenizer._emb_hash_sizes[fid_idx])
                int_emb = emb_layer(vals)  # (B, emb_dim)
            else:
                vals = user_int_feats[:, int_off:int_off + int_len].long()
                vals = _hash_ids(vals, user_ns_tokenizer._emb_hash_sizes[fid_idx])
                emb_all = emb_layer(vals)  # (B, int_len, emb_dim)
                mask = (vals != 0).float().unsqueeze(-1)
                count = mask.sum(dim=1).clamp(min=1)
                int_emb = (emb_all * mask).sum(dim=1) / count  # (B, emb_dim)

            # Dense value extraction
            if dense_len == 1:
                dense_val = user_dense_feats[:, dense_off].unsqueeze(-1)  # (B, 1)
            else:
                dense_val = user_dense_feats[:, dense_off:dense_off + dense_len].mean(dim=-1, keepdim=True)  # (B, 1)

            # Element-wise cross: int_emb * dense_val
            cross = int_emb * dense_val  # (B, emb_dim)
            cross_parts.append(cross)

        cat_cross = torch.cat(cross_parts, dim=-1)  # (B, num_shared * emb_dim)
        return F.silu(self._shared_fid_joint_proj(cat_cross))  # (B, d_model)
    ...
    def _embed_seq_domain(
        ...
        for i in range(S):
            ...
            if real_idx == -1:
                ...
                vals = seq[:, i, :].long()
                vals = _hash_ids(vals, emb_hash_sizes[i] if i < len(emb_hash_sizes) else 0)
    ...
    def _run_multi_seq_blocks(
        ...
        # Domain-level gating (if enabled)
        if self.use_domain_gate and context is not None:
            seq_tokens_list = self.domain_gate(seq_tokens_list, seq_masks_list, context)
    ...
    def _apply_second_pass_fusion(
        ...
        user_context = user_tokens.mean(dim=1)
        item_context = item_tokens.mean(dim=1)
        if self.use_target_attention:
            interest = self.target_attention(
                item_context, seq_tokens_list, seq_masks_list)
            gate = torch.sigmoid(self.target_fusion_gate)
            output = self.target_fusion_norm(output + gate * interest)

        if self.use_context_fusion:
            context_delta = self.context_fusion(
                output, user_context, item_context)
            gate = torch.sigmoid(self.context_fusion_gate)
            output = self.context_fusion_norm(output + gate * context_delta)

        if self.use_cross_fusion:
            crossed = self.cross_fusion(output)
            gate = torch.sigmoid(self.cross_fusion_gate)
            output = self.cross_fusion_norm(output + gate * (crossed - output))
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        if self.has_user_dense:
            ...
            # Shared-FID joint cross: enhance user_dense token with int-dense cross signal
            if self.use_shared_fid_joint and self._shared_fid_dense_offsets is not None:
                shared_cross = self._compute_shared_fid_cross(
                    inputs.user_int_feats, inputs.user_dense_feats, self.user_ns_tokenizer)
                gate = torch.sigmoid(self._shared_fid_gate)
                user_dense_tok = user_dense_tok + gate * shared_cross.unsqueeze(1)
            user_parts.append(user_dense_tok)
        ...
        # Compute item_context for target-aware query / domain gate (if enabled)
        item_ctx = None
        if self.use_target_aware_query or self.use_domain_gate:
            item_ctx = torch.cat(item_parts, dim=1).mean(dim=1)  # (B, D)
        q_tokens_list = self.query_generator(ns_tokens, seq_tokens_list, seq_masks_list, item_context=item_ctx)
        ...
        output = self._apply_second_pass_fusion(
            output, torch.cat(user_parts, dim=1), torch.cat(item_parts, dim=1),
