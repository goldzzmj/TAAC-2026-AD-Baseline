...
def set_seed(seed: int, full_deterministic: bool = True) -> None:
    ...
    Phase0 改动: 默认启用 full_deterministic 模式，包括:
    - cudnn.benchmark = False (禁用自动调优)
    - cudnn.deterministic = True
    - torch.use_deterministic_algorithms(True) (强制 SDPA 等算子确定性)
    - CUBLAS_WORKSPACE_CONFIG=:4096:8 (CUDA 确定性)

    注意: 这会降低训练速度，但对实验可复现性至关重要。

    Args:
        seed: Non-negative integer seed shared by all RNGs listed above.
        full_deterministic: 启用完全确定性 (Phase0 默认 True).
    """
    random.seed(seed)
    ...
    if full_deterministic:
        ...
        logging.info("Full deterministic mode enabled (cudnn.benchmark=False, "
                      "deterministic algorithms, CUBLAS_WORKSPACE_CONFIG)")
    else:
        logging.info("Partial deterministic mode (cudnn.deterministic only)")
