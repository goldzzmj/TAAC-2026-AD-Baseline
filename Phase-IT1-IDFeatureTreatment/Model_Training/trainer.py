...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        # ID feature differential LR for sparse params
        id_sparse_lr: float = 0.0,
        id_weight_decay: float = 0.1,
        id_vocab_threshold: int = 100000,
    ) -> None:
        ...
        # ID sparse params: differential LR / weight_decay when id_sparse_lr > 0
        self.id_sparse_lr: float = id_sparse_lr
        self.id_weight_decay: float = id_weight_decay
        self.id_vocab_threshold: int = id_vocab_threshold
        ...
        if hasattr(model, 'get_sparse_params'):
            ...
            if id_sparse_lr > 0 and hasattr(model, 'get_id_sparse_params'):
                id_params = model.get_id_sparse_params(id_vocab_threshold)
                normal_params = model.get_normal_sparse_params(id_vocab_threshold)
                id_param_count = sum(p.numel() for p in id_params)
                normal_param_count = sum(p.numel() for p in normal_params)
                logging.info(
                    f"ID sparse split: {len(id_params)} tensors ({id_param_count:,} params) "
                    f"with lr={id_sparse_lr}, wd={id_weight_decay}; "
                    f"{len(normal_params)} tensors ({normal_param_count:,} params) "
                    f"with lr={sparse_lr}, wd={sparse_weight_decay}")
                self.sparse_optimizer = torch.optim.Adagrad([
                    {'params': normal_params, 'lr': sparse_lr, 'weight_decay': sparse_weight_decay},
                    {'params': id_params, 'lr': id_sparse_lr, 'weight_decay': id_weight_decay},
                ])
            else:
                self.sparse_optimizer = torch.optim.Adagrad(
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            if (
                ...
                if self.id_sparse_lr > 0 and hasattr(self.model, 'get_id_sparse_params'):
                    id_params = self.model.get_id_sparse_params(self.id_vocab_threshold)
                    normal_params = self.model.get_normal_sparse_params(self.id_vocab_threshold)
                    self.sparse_optimizer = torch.optim.Adagrad([
                        {'params': normal_params, 'lr': self.sparse_lr, 'weight_decay': self.sparse_weight_decay},
                        {'params': id_params, 'lr': self.id_sparse_lr, 'weight_decay': self.id_weight_decay},
                    ])
                else:
                    self.sparse_optimizer = torch.optim.Adagrad(
