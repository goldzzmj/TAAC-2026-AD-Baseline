...
Environment variables (take precedence over CLI flags):
    TRAIN_DATA_PATH  Training parquet path (a directory or a single *.parquet file)
...
from schema_utils import ensure_schema_file
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--data_dir', type=str, default=None,
                        help='Training parquet path: a directory or a single .parquet file '
                             '(env: TRAIN_DATA_PATH)')
    parser.add_argument('--schema_path', type=str, default=None,
                        help='Schema JSON path. When omitted, an existing schema.json '
                             'is reused if present; otherwise one is auto-generated.')
    ...
    parser.add_argument('--overwrite_schema', action='store_true', default=False,
                        help='Force regeneration of schema.json even if it already exists')
    parser.add_argument('--run_name', type=str, default=None,
                        help='Optional run name recorded into saved metadata files')
    ...
    parser.add_argument('--dense_weight_decay', type=float, default=0.01,
                        help='Weight decay for dense parameters (AdamW). '
                             'Default keeps the original baseline behavior.')
    parser.add_argument('--lr_scheduler', type=str, default='none',
                        choices=['none', 'cosine'],
                        help='Dense LR schedule. cosine uses linear warmup '
                             'followed by cosine decay.')
    parser.add_argument('--warmup_steps', type=int, default=0,
                        help='Number of dense-optimizer warmup steps')
    parser.add_argument('--min_lr_ratio', type=float, default=0.1,
                        help='Minimum LR ratio used by cosine decay')
    ...
    parser.add_argument('--time_split', action='store_true', default=True,
                        help='Phase0: 按 timestamp 排序后取最新的 Row Groups 作为验证集')
    parser.add_argument('--no_time_split', dest='time_split', action='store_false',
                        help='禁用时间切分，回退到文件顺序切分')
    ...
    parser.add_argument('--use_target_attention', action='store_true', default=True,
                        help='Enable DIN-style target-aware pooling over behavior sequences')
    parser.add_argument('--no_target_attention', dest='use_target_attention',
                        action='store_false',
                        help='Disable target-aware sequence pooling')
    parser.add_argument('--target_attention_hidden_mult', type=int, default=2,
                        help='Hidden multiplier for the target-attention activation MLP')
    parser.add_argument('--use_cross_fusion', action='store_true', default=True,
                        help='Enable a small DCN-V2-style low-rank cross fusion block')
    parser.add_argument('--no_cross_fusion', dest='use_cross_fusion',
                        action='store_false',
                        help='Disable low-rank cross fusion')
    parser.add_argument('--cross_fusion_layers', type=int, default=1,
                        help='Number of low-rank cross fusion layers')
    parser.add_argument('--cross_fusion_rank', type=int, default=16,
                        help='Low-rank dimension for cross fusion')
    parser.add_argument('--use_context_fusion', action='store_true', default=True,
                        help='Enable lightweight final user/item context fusion')
    parser.add_argument('--no_context_fusion', dest='use_context_fusion',
                        action='store_false',
                        help='Disable final user/item context fusion')
    parser.add_argument('--context_fusion_hidden_mult', type=int, default=2,
                        help='Hidden multiplier for the final context-fusion MLP')
    parser.add_argument('--fusion_gate_init', type=float, default=0.1,
                        help='Initial sigmoid gate value for target/cross residual fusion')

    # Shared-FID joint modeling
    parser.add_argument('--use_shared_fid_joint', action='store_true', default=False,
                        help='Enable joint modeling of shared-fid int embeddings and dense values')
    parser.add_argument('--no_shared_fid_joint', dest='use_shared_fid_joint', action='store_false')
    parser.add_argument('--use_target_aware_query', action='store_true', default=False,
                        help='Use DIN-style target-conditioned pooling in query generator instead of MeanPool')
    parser.add_argument('--no_target_aware_query', dest='use_target_aware_query', action='store_false')
    ...
    parser.add_argument('--reinit_cardinality_threshold', type=int, default=0,
                        help='Cardinality threshold used by the re-init strategy: '
                             ...
                             'at each epoch end. 0 keeps the original baseline behavior '
                             'and resets all non-empty Embeddings; negative disables '
                             'the re-init strategy.')
    ...
    parser.add_argument('--emb_hash_size', type=int, default=0,
                        help='When --emb_skip_threshold is active, features above '
                             'the threshold are hashed into this many buckets instead '
                             'of being zeroed out (0 = keep legacy zero-vector skip).')
    ...
    # ID feature differential sparse LR.
    parser.add_argument('--id_sparse_lr', type=float, default=0.0,
                        help='Learning rate for ID sparse params (0=disabled, >0 enables ID grouping)')
    parser.add_argument('--id_weight_decay', type=float, default=0.1,
                        help='Weight decay for ID sparse params')
    parser.add_argument('--id_vocab_threshold', type=int, default=100000,
                        help='Vocab threshold to identify ID features')

    # NS tokenizer ID dropout.
    parser.add_argument('--ns_id_dropout', type=float, default=0.0,
                        help='NS tokenizer ID dropout rate')
    parser.add_argument('--ns_id_vocab_threshold', type=int, default=100000,
                        help='NS ID dropout vocab threshold')
    ...
    parser.add_argument('--use_ema', action='store_true', default=False,
                        help='Enable EMA for better generalization')
    ...
    parser.add_argument('--ema_warmup_steps', type=int, default=100,
                        help='EMA warmup steps (linear ramp from 0 to target decay)')

    # Label smoothing.
    parser.add_argument('--label_smoothing', type=float, default=0.0,
                        help='Label smoothing factor for BCE loss (0 = disabled)')
...
def _resolve_local_paths(args: argparse.Namespace) -> None:
    if not args.data_dir:
        raise ValueError(
            "Training data path is missing. Pass --data_dir or set TRAIN_DATA_PATH."
        )

    data_path = Path(args.data_dir)
    if not data_path.exists():
        raise FileNotFoundError(f"Training data path does not exist: {args.data_dir}")

    data_stem = data_path.stem if data_path.is_file() else data_path.name
    if args.ckpt_dir:
        output_root = Path(args.ckpt_dir).resolve()
    elif args.log_dir:
        output_root = Path(args.log_dir).resolve().parent
    elif args.tf_events_dir:
        output_root = Path(args.tf_events_dir).resolve().parent
    else:
        output_root = Path(__file__).resolve().parent / 'outputs' / data_stem

    if not args.ckpt_dir:
        args.ckpt_dir = str(output_root / 'checkpoints')
    if not args.log_dir:
        args.log_dir = str(output_root / 'logs')
    if not args.tf_events_dir:
        args.tf_events_dir = str(output_root / 'tensorboard')

    if args.schema_path:
        return

    if data_path.is_dir():
        existing_schema = data_path / 'schema.json'
        if existing_schema.exists():
            args.schema_path = str(existing_schema)
            return

    schema_root = Path(args.ckpt_dir).resolve() if args.ckpt_dir else output_root
    args.schema_path = str(schema_root / 'schema.json')


def _build_runtime_metadata(args: argparse.Namespace, schema_path: str) -> dict:
    return {
        ...
        "tf_events_dir": args.tf_events_dir,
        "train_env": {
...
def main() -> None:
    ...
    try:
        ...
        writer = None
        logging.warning("TensorBoard disabled because SummaryWriter failed: %s", exc)

    # ---- Data loading ----
    schema_path = ensure_schema_file(
        data_path=args.data_dir,
        schema_path=args.schema_path,
        overwrite=args.overwrite_schema,
    )
    if not args.run_name:
        args.run_name = Path(args.ckpt_dir).name

    runtime_metadata = _build_runtime_metadata(args, schema_path)
    _write_json(os.path.join(args.log_dir, 'run_metadata.json'), runtime_metadata)
    _write_json(os.path.join(args.ckpt_dir, 'run_metadata.json'), runtime_metadata)
    ...
    # ---- Shared-FID joint modeling: resolve dense offsets and int indices ----
    shared_fid_dense_offsets = None
    shared_fid_int_indices = None
    if args.use_shared_fid_joint:
        SHARED_FIDS = sorted({62, 63, 64, 65, 66, 89, 90, 91})
        # Dense offsets: from user_dense_schema entries, keyed by fid
        dense_by_fid = {}
        for fid, offset, length in pcvr_dataset.user_dense_schema.entries:
            if fid in SHARED_FIDS:
                dense_by_fid[fid] = (offset, length)
        # Int indices: position in user_int_feature_specs (by fid)
        # NOTE: user_int_feature_specs has (vocab_size, offset, length), not (fid, offset, length).
        # Must use user_int_schema.entries to get actual fids.
        int_fid_to_idx = {fid: i for i, (fid, _, _) in enumerate(pcvr_dataset.user_int_schema.entries)}
        # Build paired lists in sorted fid order to ensure alignment
        dense_offsets = []
        int_indices = []
        for fid in SHARED_FIDS:
            if fid in dense_by_fid and fid in int_fid_to_idx:
                dense_offsets.append(dense_by_fid[fid])
                int_indices.append(int_fid_to_idx[fid])
        if dense_offsets and int_indices:
            shared_fid_dense_offsets = dense_offsets
            shared_fid_int_indices = int_indices
            logging.info(
                f"Shared-FID joint: {len(dense_offsets)} shared fids "
                f"(fids={[f for f in SHARED_FIDS if f in dense_by_fid and f in int_fid_to_idx]})"
            )
        else:
            logging.warning("use_shared_fid_joint=True but no shared fids found in schema")
    ...
    model_args = {
        ...
        "emb_hash_size": args.emb_hash_size,
        "seq_id_threshold": args.seq_id_threshold,
        "ns_tokenizer_type": args.ns_tokenizer_type,
        "user_ns_tokens": args.user_ns_tokens,
        "item_ns_tokens": args.item_ns_tokens,
        "use_target_attention": args.use_target_attention,
        "target_attention_hidden_mult": args.target_attention_hidden_mult,
        "use_cross_fusion": args.use_cross_fusion,
        "cross_fusion_layers": args.cross_fusion_layers,
        "cross_fusion_rank": args.cross_fusion_rank,
        "use_context_fusion": args.use_context_fusion,
        "context_fusion_hidden_mult": args.context_fusion_hidden_mult,
        "fusion_gate_init": args.fusion_gate_init,
        "use_shared_fid_joint": args.use_shared_fid_joint,
        "shared_fid_dense_offsets": shared_fid_dense_offsets,
        "shared_fid_int_indices": shared_fid_int_indices,
        "use_target_aware_query": args.use_target_aware_query,
        "ns_id_dropout": args.ns_id_dropout,
        "ns_id_vocab_threshold": args.ns_id_vocab_threshold,
    }
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        id_sparse_lr=args.id_sparse_lr,
        id_weight_decay=args.id_weight_decay,
    ...
    final_summary = {
        ...
        "best_extra_metrics": trainer.early_stopping.best_extra_metrics,
    }
    _write_json(os.path.join(args.log_dir, 'final_status.json'), final_summary)
    _write_json(os.path.join(args.ckpt_dir, 'final_status.json'), final_summary)
