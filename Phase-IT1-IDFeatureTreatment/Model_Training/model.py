...
class GroupNSTokenizer(nn.Module):
    ...
    def __init__(self, feature_specs: List[Tuple[int, int, int]],
                 ...
                 emb_hash_size: int = 0,
                 ns_id_dropout: float = 0.0,
                 ns_id_vocab_threshold: int = 100000) -> None:
        super().__init__()
        ...
        # NS ID 特征降权: 对 vocab > threshold 的 ID 特征 embedding 额外 dropout
        self.ns_id_dropout = nn.Dropout(ns_id_dropout)
        self._is_ns_id = [vs > ns_id_vocab_threshold for vs, _, _ in feature_specs]
    ...
    def forward(self, int_feats: torch.Tensor) -> torch.Tensor:
        ...
        for group, proj in zip(self.groups, self.group_projs):
            ...
            for fid_idx in group:
                ...
                # NS ID 特征降权: 对被标记为 ID 的特征额外 dropout
                if emb_real_idx != -1 and self._is_ns_id[fid_idx] and self.training:
                    fid_emb = self.ns_id_dropout(fid_emb)
...
class RankMixerNSTokenizer(nn.Module):
    ...
    def __init__(
        ...
        emb_hash_size: int = 0,
        ns_id_dropout: float = 0.0,
        ns_id_vocab_threshold: int = 100000,
    ) -> None:
        ...
        Args:
            ...
            ns_id_dropout: Dropout rate for NS ID features (vocab > ns_id_vocab_threshold).
            ns_id_vocab_threshold: Vocab size threshold to identify ID features.
        """
        super().__init__()
        self.feature_specs = feature_specs
        self.groups = groups
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        if ns_tokenizer_type == 'group':
            ...
            self.user_ns_tokenizer = GroupNSTokenizer(
                ...
                ns_id_dropout=ns_id_dropout,
                ns_id_vocab_threshold=ns_id_vocab_threshold,
            )
    ...
    def get_id_sparse_params(self, id_vocab_threshold: int = 100000) -> List[nn.Parameter]:
        """返回 vocab > threshold 的 Embedding 参数（ID 特征）。"""
        id_ptrs = set()
        # NS tokenizer 中的 ID embedding
        for tokenizer in [self.user_ns_tokenizer, self.item_ns_tokenizer]:
            for i, (vs, offset, length) in enumerate(tokenizer.feature_specs):
                if int(vs) > id_vocab_threshold:
                    real_idx = tokenizer._emb_index[i]
                    if real_idx != -1:
                        id_ptrs.add(tokenizer.embs[real_idx].weight.data_ptr())
        # Seq embedding 中的 ID
        for domain in self.seq_domains:
            for i, vs in enumerate(self._seq_vocab_sizes[domain]):
                if int(vs) > id_vocab_threshold:
                    ...
                    if real_idx != -1:
                        id_ptrs.add(self._seq_embs[domain][real_idx].weight.data_ptr())
        return [p for p in self.parameters() if p.data_ptr() in id_ptrs]

    def get_normal_sparse_params(self, id_vocab_threshold: int = 100000) -> List[nn.Parameter]:
        """返回非 ID 的 Embedding 参数。"""
        id_ptrs = {p.data_ptr() for p in self.get_id_sparse_params(id_vocab_threshold)}
        all_sparse = self.get_sparse_params()
        return [p for p in all_sparse if p.data_ptr() not in id_ptrs]
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # Compute item_context for target-aware query (if enabled)
        item_ctx = None
        if self.use_target_aware_query:
