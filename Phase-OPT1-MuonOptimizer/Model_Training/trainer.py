...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        # When dense_optimizer_type='muon', 2D params (Linear weights) use Muon,
        # and 1D params (bias, LayerNorm) use AdamW.
        self.sparse_optimizer: Optional[torch.optim.Optimizer]
        self.muon_optimizer: Optional[Muon] = None
        self.muon_scheduler: Optional[torch.optim.lr_scheduler.LambdaLR] = None
        self.dense_optimizer_type = dense_optimizer_type
        ...
        if hasattr(model, 'get_sparse_params'):
            ...
            if dense_optimizer_type == 'muon':
                # Split dense params into 2D (Linear weights) and 1D (bias/LN)
                params_2d = [p for p in dense_params if p.dim() >= 2]
                params_1d = [p for p in dense_params if p.dim() < 2]
                count_2d = sum(p.numel() for p in params_2d)
                count_1d = sum(p.numel() for p in params_1d)
                logging.info(
                    f"Dense params (Muon split): {len(params_2d)} 2D tensors "
                    f"({count_2d:,} params -> Muon lr={muon_lr}, momentum={muon_momentum}), "
                    f"{len(params_1d)} 1D tensors ({count_1d:,} params -> AdamW lr={lr})")

                self.dense_optimizer = torch.optim.AdamW(
                    params_1d, lr=lr, betas=(0.9, 0.98),
            ...
            if dense_optimizer_type == 'muon':
                # Split all model parameters into 2D and 1D
                all_params = list(model.parameters())
                params_2d = [p for p in all_params if p.dim() >= 2]
                params_1d = [p for p in all_params if p.dim() < 2]
                self.dense_optimizer = torch.optim.AdamW(
                    params_1d, lr=lr, betas=(0.9, 0.98),
                    weight_decay=dense_weight_decay,
                )
        ...
        if lr_scheduler == 'cosine':
            ...
            # Cosine LR for Muon optimizer (2D params) as well
            if self.muon_optimizer is not None:
                self.muon_scheduler = torch.optim.lr_scheduler.LambdaLR(
                    self.muon_optimizer, lr_lambda=lr_lambda)
                logging.info(
                    f"Using cosine Muon LR scheduler: total_steps={total_steps}, "
                    f"warmup_steps={self.warmup_steps}, min_lr_ratio={self.min_lr_ratio}")
        ...
        logging.info(f"PCVRHyFormerRankingTrainer loss_type={loss_type}, "
                     ...
                     f"dense_optimizer_type={dense_optimizer_type}")
